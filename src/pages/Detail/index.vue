        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- 
                  输入框的blur事件，是失去焦点的事件，只要添加了它，失去焦点就会触发这个事件
                  但是不好，因为它不会去判断这次输入的数据和之前的数据是不是一样再触发

                  输入框的change事件内部包含了失去焦点的事件，当用户失去焦点的时候，也会选择性触发change事件
              ...
              <div class="add">
                <!-- 
                  之前我们的跳转几乎都是点击直接跳转，因为我们再跳转之前发请求
                  比如home去search  从search去详情，跳转时候不需要发送请求，而是跳转过去后发请求拿数据

                  而加入购物车不一样，当我们点击添加购物车的时候，我们得需要先发请求给后台，后台需要把这个购物车
                  信息存储数据库，请求成功后，会返回给我们信息。我们根据这个信息再去跳转
                  否则，又可能后台添加数据失败，而我们已经跳转到下个页面，就出问题
...
export default {
  ...
  methods: {
    ...
    changeChecked(spuSaleAttrValueList, spuSaleAttrValue) {
      ...
      // spuSaleAttrValue代表点击的那个一个属性值
      // 排他第一步：让所有的成员全部都变成同一种转台
      spuSaleAttrValueList.forEach((item) => {
        item.isChecked = "0";
      });
      // 排他第二步：让当前点击的这一个成员变为另一种状态
      spuSaleAttrValue.isChecked = "1";
    },
    // 点击添加购物车逻辑
    async addShopCart(){
      // dispatch的意识是  触发分发的意思，本质就是在调用相关的action函数
      // 现在我想要在组件这里知道请求是成功的还是失败的
      // 我们只能通功dispatch的返回值，来判断请求时成功的还是失败的
      // 目前我们下面这行代码，结果一定是promise

      // 第一种写法  action的异步函数，返回的promise永远是成功的，要看action异步函数的return
      // 第一步：现发送请求添加购物车
      /* const result = await this.$store.dispatch('reqAddOrUpdateShopCart',{skuId:this.skuId,skuNum:this.skuNum})
      // 第二步：根据请求添加购物车返回的信息决定是否跳转到添加购物车成功页面
      if(result == 'ok'){
        alert('添加购物车成功，将前往购物车页面')
      }else{
        alert('添加购物车失败！请重新添加')
      } */

      // 第二种写法  action的异步函数，返回的promise可能是成功也可能失败，成功就代表请求成功，返回结果ok, 失败就拿到失败的promise的原因
      try{
        // 成功
        await this.$store.dispatch('addOrUpdateShopCart',{skuId:this.skuId,skuNum:this.skuNum});
        //往添加购物车成功页面跳转的时候，需要带两个东西，skuNum和商品信息
        // skuNum是一个简单数据，我们直接通过路由传参query参数带过去
        // 商品信息是一个复杂的数据，我们最好不要通过路由传参传递对象，因此我们采用存储的方案，sessionStorage
        alert('添加购物车成功，将前往购物车页面');
        // loaclStorage(真正的永久存储) sessionStorage（关闭浏览器数据就不在了）
