...
export default {
  ...
  mounted() {
    this.reqPayInfo(this.orderId);
  },
  methods: {
    async reqPayInfo(orderId) {
    ...
    async pay() {
      ...
      this.$alert(`<img src='${imgUrl}'>`, "请使用微信支付", {
        ...
        showClose:false,
        center:true,
        // beforeClose一旦被设定，那么消息盒子关闭不关闭，我们呢在这个里面进行控制
        beforeClose:(action,instance,done)=>{
          if(action==='confirm'){
            //代表用户点击的确定
            // if(!this.payStatus){
            //   this.$message.info('请确保支付成功，成功后会自动跳转支付成功界面')
            // }
            // 清除定时器
            ...
            this.$message.warning('支付问题请联系龙湖管家');
            // 清除定时器
            clearInterval(this.timer);
            this.timer = null;
            // 关闭消息盒子
        ...
        ()=>{
          //.then()是应对点击确定按钮之后的操作
          // 如果采用这个两个去操作，都会强制关闭我们弹出框msgBox
          // 而我们闲杂闭包那个不是要直接关闭，是需要判断用户是不是支付了，才决定关不关闭
        }
      ).catch(
        ()=>{
          // .catch()是应对点击取消按钮之后的操作
        }
      );
      if(!this.timer){
